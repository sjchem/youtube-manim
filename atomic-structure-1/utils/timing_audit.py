...
def camera_class_for(scene_key: str) -> type[Camera]:
    """3D chapters need a ThreeDCamera; a passed-in renderer ignores camera_class."""
    return ThreeDCamera if scene_key[-2:] in cfg.THREE_D_SCENES else Camera
...
def audit_scene(scene_name: str) -> TimingResult:
    ...
    try:
        renderer = TimingRenderer(camera_class=camera_class_for(key))
...
def main() -> int:
    ...
    parser.add_argument("scenes", nargs="*", help="Scene keys such as scene_04 or scene_04_slope")
    parser.add_argument("--fps", type=int, default=15)
    args = parser.parse_args()

    manim_config.frame_rate = args.fps
    manim_config.progress_bar = "none"
    normalized = [normalize(name) for name in (args.scenes or list(SCENE_MAP))]
