...
import numpy as np
from manim import MathTex, Scene, Text, ThreeDCamera, config as manim_config
    ...
    for top_level in scene.mobjects:
        ...
        for member in top_level.get_family():
            if id(member) in seen or not isinstance(member, (Text, MathTex)):
                continue
            if getattr(member, "_is_decorative_shadow", False):
                continue
            seen.add(id(member))
...
class LayoutRenderer(CairoRenderer):
    ...
    def __init__(self, scene_key: str, camera_class: type[Camera] = Camera) -> None:
...
def main() -> int:
    ...
    for scene_name in selected:
        ...
        key = scene_name[-2:]
        camera_class = ThreeDCamera if key in cfg.THREE_D_SCENES else Camera
        try:
            renderer = LayoutRenderer(scene_name, camera_class)
