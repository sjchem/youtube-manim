"""Safe-frame and zoom helpers for mobile-readable compositions."""
...
def zoom_factor_sequence(levels: tuple[float, ...] = (1.0, 5.0, 20.0, 100.0, 1000.0)) -> tuple[float, ...]:
    """Successive scale multipliers to go from one zoom level to the next."""
    return tuple(levels[i + 1] / levels[i] for i in range(len(levels) - 1))
