...
from manim_scenes.common import (
    add_cinematic_background, begin_scene, end_scene, narration_wait, paced_play,
)
...
def play_scene(scene: Scene) -> None:
    scene_start = begin_scene(scene, "14")
    ...
    paced_play(scene, Flash(subscribe.get_center(), color=cfg.GOLD, line_length=0.5, num_lines=16), run_time=1.0)
    narration_wait(scene, 1.4)
    paced_play(scene, Indicate(thank_you, color=cfg.WHITE, scale_factor=1.05), run_time=0.9)
    narration_wait(scene, 2.6)
    paced_play(scene, Indicate(subscribe, color=cfg.CYAN, scale_factor=1.05), run_time=0.9)
    narration_wait(scene, 1.6)
    paced_play(scene, Indicate(tagline, color=cfg.CYAN, scale_factor=1.03), run_time=0.9)
    end_scene(scene, scene_start, cfg.SCENE_DURATIONS["14"])
